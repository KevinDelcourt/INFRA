...
public class CycleByCycleStrategy implements SchedulingStrategy {
        ...
        while(isRunning) {
            synchronized (this) {
                ...
                while (this.currentAgentCycle < this.maxCycleAgent && listAgentsToSchedule.size()>0 ) {
                    ...
                    currentInfraAgent.run(); //Launch the behavior of the agent in its PERCEPTION irit.infra.State
                    currentInfraAgent.run(); //Launch the behavior of the agent in its DECISION irit.infra.State
                    currentInfraAgent.run(); //Launch the behavior of the agent in its ACTION irit.infra.State
